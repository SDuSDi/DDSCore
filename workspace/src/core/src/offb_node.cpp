...
public:
    ControlNode() : Node("ddscore")
    {
        ...
        service_commander = this -> create_subscription<std_msgs::msg::Empty>("/ddscore/takeoff",qos, 
                            std::bind(&ControlNode::publish_srv_command, this, std::placeholders::_1));
    }
...
private:
    ...
    void publish_srv_command(const std_msgs::msg::Empty::SharedPtr msg);
    ...
    rclcpp::Subscription<std_msgs::msg::Empty>::SharedPtr service_commander;
...
void ControlNode::cb_srv_command(const rclcpp::Client<px4_msgs::srv::VehicleCommand>::SharedFuture future) {
    ...
    if (status == std::future_status::ready) {
        ...
        std::to_string(future.get() -> reply.command));
    } else {
        RCLCPP_INFO(this -> get_logger(), "Service in progress");
    }
}

void ControlNode::publish_srv_command(const std_msgs::msg::Empty::SharedPtr msg) {
	...
	command.param7 = global_pos[3] + 2.50; // Absolute altitude + takeoff altitude
...
// void ControlNode::set_takeoff(const std_msgs::msg::Empty::SharedPtr msg) {
//     (void) msg; //Shuts the compiler up about the empty message not being used
