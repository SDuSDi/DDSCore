  ...
  core_msgs__msg__AuxGlobalPosition__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    core_msgs__msg__AuxGlobalPosition * data =
      (core_msgs__msg__AuxGlobalPosition *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!core_msgs__msg__AuxGlobalPosition__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          core_msgs__msg__AuxGlobalPosition__fini(&data[i]);
        }
        free(data);
        return false;
      }
