...
source /opt/ros/humble/setup.bash && cd workspace && colcon build && source install/setup.bash && ros2 run link node &
cd workspace && source install/setup.bash && ros2 run core node